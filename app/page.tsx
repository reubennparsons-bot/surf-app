...
const SKILL_OPTIONS: { value: SkillLevel; label: string; hint: string }[] = [
  { value: 'beginner', label: 'Beginner', hint: 'Foam top, whitewater, lessons' },
  { value: 'improver', label: 'Improver', hint: 'Catching unbroken waves, learning turns' },
  { value: 'intermediate', label: 'Intermediate', hint: 'Comfortable head-high; reads lineup' },
  { value: 'advanced', label: 'Advanced', hint: 'Overhead, reefs, powerful waves' },
];
...
export default function Home() {
  ...
  const [specificIso, setSpecificIso] = useState('');
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    const body: unknown = {
      ...
      skill,
      timing:
        timing === 'specific' && specificIso
          ? { kind: 'specific', iso: specificIso }
          : { kind: timing === 'specific' ? 'today' : timing },
    };
    ...
    <div className="mx-auto flex w-full max-w-3xl flex-col gap-6 px-4 py-8 sm:px-6 sm:py-12">
      ...
      {!showResults && (
        <form onSubmit={handleSubmit} className="flex flex-col gap-5 rounded-xl border border-zinc-200 bg-white p-5 shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
          ...
          <div>
            ...
            <div className="mt-1.5 flex gap-2">
              <input
                ...
                className="flex-1 rounded-md border border-zinc-300 bg-white px-3 py-2 text-sm text-zinc-900 placeholder:text-zinc-400 focus:border-zinc-600 focus:outline-none focus:ring-1 focus:ring-zinc-600 dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-100"
                required
              />
              <button
                type="button"
                onClick={useGeolocation}
                className="shrink-0 rounded-md border border-zinc-300 px-3 py-2 text-sm font-medium text-zinc-700 transition-colors hover:bg-zinc-100 dark:border-zinc-700 dark:text-zinc-300 dark:hover:bg-zinc-900"
          ...
          <fieldset>
            ...
            {timing === 'specific' && (
              <input
                type="datetime-local"
                value={specificIso}
                onChange={(e) => setSpecificIso(e.target.value)}
          ...
          >
            {phase === 'loading' ? 'Checking conditions…' : 'Get recommendations'}
          </button>
        </form>
      )}
